...
if [ `hostname` == "gen-l-vrt-010.mtl.labs.mlnx" ]; then
    ...
    printf -v CORE_LIST "%s," "${RCORES[@]:0:6}"
    CORE_LIST=${CORE_LIST%?}
    echo ssh -t $loader1 sudo set_irq_affinity_cpulist.sh $CORE_LIST $dif1
    ssh -t $loader1 sudo set_irq_affinity_cpulist.sh $CORE_LIST $dif1
else
...
if [ $ISENC == 1 ]; then
	...
	echo LD_LIBRARY_PATH=$OPENSSL taskset -c 0 $perf stat -D $[$RAMP*1000+1000] -a -C 0 -e task-clock,cycles,instructions,cache-misses -x, --append -o $OUTPUT/perf_stat.txt $IPERF/src/iperf -s --ktls 
	LD_LIBRARY_PATH=$OPENSSL taskset -c 0 $perf stat -D $[$RAMP*1000+1000] -a -C 0 -e task-clock,cycles,instructions,cache-misses -x, --append -o $OUTPUT/perf_stat.txt $IPERF/src/iperf -s -l $MSGSIZE --ktls &
...
echo "*********************************##################################" | tee -a $OUTPUT/iperf.txt
if [ $ISENC == 1 ]; then
	for i in `seq 1 $THREADS`;
	do
		echo "LD_LIBRARY_PATH=$OPENSSL taskset -c ${LCORES[$i]} $perf stat -D $[$RAMP*1000] -a -C ${LCORES[$i]} -e task-clock,cycles,instructions,cache-misses -x, --append -o $OUTPUT/perf_stat.txt $IPERF/src/iperf -c $dip1 -l $MSGSIZE -t $[$TIME+$RAMP] --tls --ktls --ktls_record_size=$RECSIZE -p $[$PORT + $i] | tee -a $OUTPUT/iperf.txt &"
		LD_LIBRARY_PATH=$OPENSSL taskset -c ${LCORES[$i]} $perf stat -D $[$RAMP*1000] -a -C ${LCORES[$i]} -e task-clock,cycles,instructions,cache-misses -x, --append -o $OUTPUT/perf_stat.txt $IPERF/src/iperf -c $dip1 -l $MSGSIZE -t $[$TIME+$RAMP] --tls --ktls --ktls_record_size=$RECSIZE -p $[$PORT + $i] | tee -a $OUTPUT/iperf.txt &
	done
