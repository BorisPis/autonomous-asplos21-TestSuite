...
set output "nginx3_tls.eps"

#-------------------------------------------------------------------------------
# dimensions
#-------------------------------------------------------------------------------
xsiz = .25;  # x size of each subfig
...
#-------------------------------------------------------------------------------
set xtics ("16" 0,"64" 1,"256" 2,"1024" 3) out nomirror \
...
set label 1 "file size [KiB]" \
    at screen (xoff+xsiz*xnum/2), screen 0.06 center
...
set key at first -1.0, first -31.0 samplen .5 reverse left Left maxcols 4 maxrows 1 width -1.7
...
i=1  ; arYhi[i]=  55; arYtic[i]=10;  arCores[i]=1; 
...
do for [k=1:3] {
  ...
  plot \
    ...
    f u    (yv(column(c0+1)  ,column(c1))) t "https+off" , \
    f u    (yv(column(c0+2)  ,column(c1))) t "https+off+zc" , \
    f u    (yv(column(c0+3)  ,column(c1))) t "http" , \
    f u 0:(fix(column(higher),column(c1))):(lb(column(c0+0),column(c0+2))) \
        not w labels offset 0,.8 rotate by 0 textcolor 'grey40' 
