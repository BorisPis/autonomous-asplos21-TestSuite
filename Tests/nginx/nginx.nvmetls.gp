...
# functions
#-------------------------------------------------------------------------------
#labstr(n,d)  = (d==0 ? "" : ((n/d >= 1.005 || n/d < .995)  \
...
do for [k=1:2] {
  ...
  f = sprintf("result.nvmetls.s.%s.csv", id);
  plot \
    maxy(x) not ls 5 w line, \
    f u (f($4)) t "baseline", \
    f u (f($5)) t "offload" , \
    f u 0:(f($5)):(labstr($5,$4)) not w labels offset 0,.95 rotate by 0 \
      textcolor 'grey40'
#  plot \
#    maxy(x) not ls 5 w line, \
#    f u (f($2)) t "baseline", \
#    f u (f($3)) t "offload" , \
#    f u 0:(f($3)):(labstr($3,$2)) not w labels offset 0,.6 rotate by 0 \
#      textcolor 'grey40'
...
do for [k=1:2] {
  ...
  f = sprintf("result.nvmetls.s.%s.csv", id);
  plot \
    f u (idle($2,$7)) t "baseline", \
    f u (idle($3,$7)) t "offload" , \
    f u 0:(idle($3,$7)):(labstr2($2,$3,$7)) not w labels offset 0,.6 \
