...
#-------------------------------------------------------------------------------
set xtics ("4" 0,"16" 1,"64" 2,"256" 3, "1Ki" 4) out nomirror \
    scale .7 offset 0,.2 rotate by -45; # font ",16"
set xrange [-.70:4.7]
...
i=1  ;  arYtic[i]=2; arCores[i]=1;  arYhi[i]= legend_inside ? 12.5 : 10; 
i=i+1;  arYtic[i]=5; arCores[i]=8;  arYhi[i]= 28;
i=i+1;  arYtic[i]=2; arCores[i]=8;  arYhi[i]=  9;
...
do for [k=1:3] {
  ...
  c0        = (k<=2) ?  2 : 4;	# which column in the csv
  c1        = (k<=2) ? 11 : 9;	# which column in the csv
  ...
  f         = sprintf("result.%d.s.csv", cores);
  ...
  plot \
    ...
    f u 0:(fix(column(higher),column(c1))):(lb(column(c0+0),column(c0+1))) \
        not w labels offset 0,.7 rotate by 35 textcolor 'grey40' 
