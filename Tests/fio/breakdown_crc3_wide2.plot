...
set output 'breakdown_crc3_wide2.eps'
...
set xtics ('1' 0, '2' 1, '4' 2, '8' 3, '16' 4, '32' 5, '64' 6, '128' 7, '256' 8, '512' 9, '1Ki' 10, '2Ki' 11, '4Ki' 12, '8Ki' 13);
set xrange [-.75:13.75]
...
t=4; set style line t lt -1 lw 5 pt t ps 1.2
t=5; set style line t lt -1 lw 5 pt 1 ps 1.2
t=6; set style line t lt -1 lw 5 pt 2 ps 1.2

#-------------------------------------------------------------------------------
# labels + key
#-------------------------------------------------------------------------------
set label 1 at screen 0.02, screen (yoff+ysiz*ynum/2) center \
    rotate "cycles per IO"
...
arr_other0 =      "0     0     0     0    0   0"; 
arr_other =       "30     39     77     220    830   3300"; 
...
do for [k=1:4] {
    ...
    min_other0 = word(arr_other0,k);    # where the first ytic will appear
    ...
    yhi0      = min_other * 3.6;      # top end of yrange
    ...
    yic0       = sprintf(yticfmt, norma(min_other0));
    ...
    if( yic > 0 ) {set ytics ( "%.0f" yic0, yic.yticsuffix yic, yic4.yticsuffix  yic4)  } else {set ytics autofr}

    set ytics format "%g".yticsuffix offset .7,0 font ", 15"
    ...
    '' u 0:(norma(  allmnus($4,$2,$3,$8,yhi0))):(labstr(cpy_prcnt($4,$2,$3)))                   \
                                          ls 4 not            w labels rotate by 30, \
