...
set label 1 at screen 0.02, screen (yoff+ysiz*ynum/2) center \
    rotate "cycles per messsage"
#set label 2 at screen (xoff+xsiz*xnum/2), screen .07 center \
#    'TLS record size' 

#set key at screen (xall/4), -0.002 Left left bottom \
...
# NVME data functions
#-------------------------------------------------------------------------------
other2    (cycall,cyccpy,cyccrc,iops)     = (cycall - cyccpy) / iops;
copy2     (cycall,cyccpy,cyccrc,iops)     = (cyccpy / iops);
allplus2  (cycall,cyccpy,cyccrc,iops,yhi) = (cycall / iops) + .10*yhi;               	# plus for labe
cpy_prcnt2(cycall,cyccpy,cyccrc)     = (cyccpy / cycall) * 100.0

#-------------------------------------------------------------------------------
# arrays -- one entry per plot
#-------------------------------------------------------------------------------
# isenc?
arr_ids = " NVME-TCP TLS ";
...
do for [k=1:2] {
    ...
    if (k==2) {
	set key at screen (xall/4)+0.1, -0.002 Left left bottom \
	    samplen 2 spacing 2.9 width 0 reverse \
	    maxrows 1
	    yhi0      = 50 * 1000
	    yhi      = 50 
	    set yrange[0:yhi]
# This is TLS
#test                      bs           enc_cyc         opcyc       iperf_thpt     enc_per_rec          xtic
	    plot sprintf("model.tls.csv") \
	       u   (norma(    other($2,$3,$4,$5    )))  ls 1   t 'other'            , \
	    '' u   (norma(      enc($2,$3,$4,$5    )))  ls 3   t 'crypto'         , \
	...
	set key at screen (xall/4)-0.1, -0.002 Left left bottom \
	    ...
	    set yrange[0:yhi]
# This is NVME
	    plot sprintf("model.nvme.csv") \
	       u   (norma2(    other2($4,$2,$3,$8    )))  ls 1   not                    , \
	    '' u   (norma2(     copy2($4,$2,$3,$8    )))  ls 2   t 'crc+copy'         , \
	    '' u 0:(norma2(  allplus2($4,$2,$3,$8,yhi0))):(labstr(cpy_prcnt2($4,$2,$3)))                   \
						  ls 3 not            w labels
    }
